...
# ── ALWAYS load zarr — needed for full time series loop regardless of cache ────
...
print(f"  {len(dates)} time steps loaded: {dates[0].date()} → {dates[-1].date()}")

# ── load or compute outbreak window scores ─────────────────────────────────────
if os.path.exists(VALIDATION_CACHE):
    ...
    # find time steps covering 2019–2021 outbreak period
    target_indices = [
...
# ── MAP 7: spatial comparison before / during / after ─────────────────────────
...
before_idx = 0
during_idx = peak_idx
after_idx  = len(target_indices) - 1
...
# ── full 2017–2025 time series ─────────────────────────────────────────────────
print("\nComputing full 2017–2025 time series...")
all_scores = []
all_dates  = []
...
# ── MAP 11: full time series ───────────────────────────────────────────────────
full_peak_idx  = int(np.argmax(all_scores))
full_peak_date = all_dates[full_peak_idx]

fig, ax = plt.subplots(figsize=(18, 6))
fig.suptitle(
    "Full 2017–2025 stress score time series\n"
    "Does January–March peak repeat every year?",
    fontsize=13, fontweight="bold"
)
...
ax.annotate(
    f"Highest peak\n{full_peak_date.date()}\n{all_scores[full_peak_idx]:.1f}%",
    xy=(full_peak_date, all_scores[full_peak_idx]),
    xytext=(full_peak_date + pd.Timedelta(days=60), all_scores[full_peak_idx] - 3),
...
print(f"  Overall peak: {full_peak_date.date()} at {all_scores[full_peak_idx]:.1f}%")
print("────────────────────────────────────────────────────────────")
...
print(f"  Literature peak window:  January–March")
print(f"  Your model peak:         {peak_date.strftime('%B %Y')}")
if peak_date.month in [1, 2, 3]:
    print(f"  ✓ Peak falls exactly within documented CLR peak window")
if pd.Timestamp("2020-01-01") <= peak_date <= pd.Timestamp("2022-01-01"):
    print(f"  ✓ Peak within documented outbreak window — model validated")
print("────────────────────────────────────────────────────────────")